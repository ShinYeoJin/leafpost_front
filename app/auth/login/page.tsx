...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log("[LoginPage] 참고: 쿠키 기반 인증이므로 response body는 선택적입니다");
      
      // ✅ 쿠키가 브라우저에 반영되도록 충분한 대기
      // sameSite: 'none' 쿠키는 크로스 도메인 설정이므로 브라우저 처리 시간이 필요함
      // login 함수 내부에서 이미 1500ms 대기하므로 추가 대기
      // 크로스 도메인 쿠키는 더 많은 시간이 필요할 수 있음
      console.log("[LoginPage] 쿠키 반영 대기 중... (크로스 도메인 쿠키는 시간이 필요할 수 있음)");
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      console.log("[LoginPage] ✅ 쿠키 반영 대기 완료 - /main으로 리다이렉트");
      console.log("[LoginPage] 참고: httpOnly 쿠키는 document.cookie에서 보이지 않지만, 브라우저는 자동으로 포함합니다.");
      console.log("[LoginPage] middleware에서 쿠키를 확인하여 인증 상태를 판단합니다.");
